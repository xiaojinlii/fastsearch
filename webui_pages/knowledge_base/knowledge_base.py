...
from application.settings import ZH_TITLE_ENHANCE, OVERLAP_SIZE, CHUNK_SIZE, VECTOR_DB, DEFAULT_VS_TYPE, LOADER_DICT
from webui_pages.api_request import ApiRequest, check_error_msg, check_success_msg

cell_renderer = JsCode("""function(params) {if(params.value==true){return '✓'}else{return '×'}}""")

vs_types = [key for key in VECTOR_DB.keys()]
...
def knowledge_base_page(api: ApiRequest):
    try:
        kb_list = {x["kb_name"]: x for x in api.list_knowledge_base_details()}
    ...
    if selected_kb == "新建知识库":
        ...
        doc_details = pd.DataFrame(api.list_kb_file_details(kb))
        selected_rows = []
        if not len(doc_details):
            ...
            if cols[1].button(
                ...
                file_names = [row["file_name"] for row in selected_rows]
                api.delete_files(kb, file_names=file_names, delete_content=True)
                st.rerun()
        ...
        if selected_rows:
            ...
            docs = api.list_file_docs(knowledge_base_name=selected_kb, file_name=file_name)
            data = [
                {
                    "No": i + 1,
